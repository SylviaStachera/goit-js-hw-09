...
Create an HTML file (01-color-switcher.html) and a JavaScript file (01-color-switcher.js).
Add two buttons to the HTML file: "Start" and "Stop".
When the "Start" button is clicked, change the background color of the <body> element to a random color every second using inline styles. Disable the "Start" button while the color change is active.
When the "Stop" button is clicked, the color change should stop and the background color should remain at the current color.

## Task 2 - Countdown Timer:
Create an HTML file (02-timer.html) and a JavaScript file (02-timer.js).
Add an input field for selecting the end date and a "Start" button to the HTML file.
Use the flatpickr library to enable date and time selection in the input field.
When a valid future date is selected, enable the "Start" button. If a past date is selected, display an alert message asking the user to choose a future date.
After clicking the "Start" button, start the countdown timer by displaying the remaining time (days, hours, minutes, and seconds) in the specified format (DD:HH:MM:SS).
The countdown should stop when the timer reaches 00:00:00:00.

## Task 3 - Promises Generator:
Create an HTML file (03-promises.html) and a JavaScript file (03-promises.js).
Add a form with input fields for specifying the initial delay, delay step, and the number of promises to create.
When the form is submitted, call the createPromise function multiple times based on the user input. Each promise should have a different position and delay value, incremented according to the specified step.
The createPromise function should return a promise that resolves or rejects randomly. Handle the fulfilled and rejected promises accordingly.

Note: The use of the Notiflix library is optional for displaying notifications or alerts.
